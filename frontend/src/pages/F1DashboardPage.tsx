...
function F1DashboardPage() {
  ...
  useEffect(() => {
    ...
    const findLatestWithResults = async () => {
      for (const season of seasons) {
        try {
          const seasonEvents = await getEvents('f1', season.id);
          const hasCompleted = seasonEvents.some((e) => e.status === 'completed');
          if (hasCompleted) {
  ...
  const handleYearChange = (year: string) => {
    navigate(`/f1?year=${year}`, { replace: true });
  };

  return (
    <AppLayout title="🏎️ F1 Dashboard" subtitle="Formula 1 Racing Statistics & Data">
      <div className="mb-8">
...
export default F1DashboardPage;
